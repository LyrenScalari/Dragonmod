    ...
    public void update() {
        if (this.duration == this.startDuration) {
            if (AbstractDungeon.player.hasPower(InspirationPower.POWER_ID)) {
                if (ExaltCost.get(TypeEnergyHelper.Mana.Exalt) <= AbstractDungeon.player.getPower(InspirationPower.POWER_ID).amount) {
                    AbstractPower reduceMe = AbstractDungeon.player.getPower(InspirationPower.POWER_ID);
                    if (reduceMe != null) {
