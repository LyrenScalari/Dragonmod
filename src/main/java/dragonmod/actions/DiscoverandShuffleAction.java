...
public class DiscoverandShuffleAction
        extends AbstractGameAction
{
    ...
    public DiscoverandShuffleAction(CardGroup group)
    {
        this(group, 3, false, null);
    }

    public DiscoverandShuffleAction(CardGroup group, int number, boolean allowSkip, Consumer<AbstractCard> callback)
    {
        this.group = group;
    ...
    public DiscoverandShuffleAction(CardGroup group, int number)
    {
            ...
            if (!this.retrieveCard) {
                if (AbstractDungeon.cardRewardScreen.discoveryCard != null) {
                    ...
                    if (AbstractDungeon.player.hand.size() < BaseMod.MAX_HAND_SIZE) {
                        AbstractDungeon.effectList.add(new ShowCardAndAddToDrawPileEffect(disCard, (float)Settings.WIDTH / 2.0F, (float)Settings.HEIGHT / 2.0F,true));
                    } else {
                        AbstractDungeon.effectList.add(new ShowCardAndAddToDrawPileEffect(disCard, (float)Settings.WIDTH / 2.0F, (float)Settings.HEIGHT / 2.0F,true));
                    }
