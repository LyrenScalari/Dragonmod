...
public class ThrowIceDaggerEffect extends AbstractGameEffect {
    private float x;
    private float y;
    private float destY;
    ...
    public ThrowIceDaggerEffect(float x, float y) {
    ...
    public ThrowIceDaggerEffect(float x, float y, float fAngle) {
        this.img = ImageMaster.DAGGER_STREAK;
        this.x = x - MathUtils.random(320.0F, 360.0F) - (float)this.img.packedWidth / 2.0F;
        this.destY = y;
        this.y = this.destY + MathUtils.random(-25.0F, 25.0F) * Settings.scale - (float)this.img.packedHeight / 2.0F;
