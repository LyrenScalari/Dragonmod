...
public class NightTerror extends AbstractWardenCard implements onAttackedField, BranchingUpgradesCard {


    // TEXT DECLARATION

    public static final String ID = DragonMod.makeID(NightTerror.class.getSimpleName()); // USE THIS ONE FOR THE TEMPLATE;
    ...
    public NightTerror(){
        ...
        energyCosts.put(TypeEnergyHelper.Mana.Temporal,2);
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        addToBot(new AbstractGameAction() {
            @Override
            public void update() {
                FieldsManager.addCard(NightTerror.this,true,p);
                isDone = true;
            }
        });
    }
        ...
        if (AbstractDungeon.actionManager.turnHasEnded && info.owner != owner && dmg > 0){
            ...
            if (energyCosts.get(TypeEnergyHelper.Mana.Temporal) < 1){
                Wiz.atb(new AbstractGameAction() {
                    @Override
                    public void update() {
                        FieldsField.Fields.get(AbstractDungeon.player).remove(NightTerror.this);
                        Wiz.atb(new UnlimboAction(NightTerror.this));
                        if (!upgraded){
                            AbstractDungeon.player.discardPile.moveToExhaustPile(NightTerror.this);
                        } else {
                            AbstractDungeon.player.discardPile.moveToDiscardPile(NightTerror.this);
                        }
                        this.isDone = true;
                    }
                });
