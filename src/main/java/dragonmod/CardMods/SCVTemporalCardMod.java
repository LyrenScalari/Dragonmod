...
public class SCVTemporalCardMod extends AbstractCardModifier {
    ...
    public void onSingleCardViewRender(AbstractCard card, SpriteBatch sb) {
        if (card instanceof AbstractWardenCard){
            ExtraIcons.icon(TemporalIcon.get().region.getTexture()).text(String.valueOf(((AbstractWardenCard)card).energyCosts.get(TypeEnergyHelper.Mana.Temporal))).render(card);
        }
    }
