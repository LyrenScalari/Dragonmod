...
public class CardArtRoller {
    ...
    public static void computeCard(AbstractEasyCard c) {
        c.portrait = doneCards.computeIfAbsent(c.cardID, key -> {
            ReskinInfo r = infos.computeIfAbsent(key, key2 -> {
                ...
                ArrayList<AbstractCard> cardsList = DragonMod.util.Wiz.getCardsMatchingPredicate(s -> s.type == c.type && WhatMod.findModName(s.getClass()) == null, true);
                String q = DragonMod.util.Wiz.getRandomItem(cardsList, rng).cardID;
            ...
            t.flip(false, true);
            FrameBuffer fb = DragonMod.util.ImageHelper.createBuffer(250, 190);
            OrthographicCamera og = new OrthographicCamera(250, 190);
            SpriteBatch sb = new SpriteBatch();
            sb.setProjectionMatrix(og.combined);
            DragonMod.util.ImageHelper.beginBuffer(fb);
            sb.setShader(shade);
            sb.setColor(HSLC);
            sb.begin();
            sb.draw(t, -125, -95);
            ...
            fb.end();
            t.flip(false, true);
            TextureRegion a = DragonMod.util.ImageHelper.getBufferTexture(fb);
    ...
    public static Texture getPortraitTexture(AbstractCard c) {
        ...
        Color HSLC = new Color(r.H, r.S, r.L, r.C);
        TextureAtlas.AtlasRegion t = new TextureAtlas.AtlasRegion(DragonMod.util.TexLoader.getTexture("images/1024Portraits/" + CardLibrary.getCard(r.origCardID).assetUrl + ".png"), 0, 0, 500, 380);
        t.flip(false, true);
        FrameBuffer fb = DragonMod.util.ImageHelper.createBuffer(500, 380);
